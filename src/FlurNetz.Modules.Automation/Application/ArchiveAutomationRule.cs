...
/// <summary>Archiviert eine Automation-Rule terminal.</summary>
public sealed class ArchiveAutomationRule
{
    private readonly IAutomationRuleStore store;
    private readonly IClock clock;

    /// <summary>Erstellt den Use Case.</summary>
    public ArchiveAutomationRule(IAutomationRuleStore store, IClock clock)
    {
        this.store = store ?? throw new ArgumentNullException(nameof(store));
        this.clock = clock ?? throw new ArgumentNullException(nameof(clock));
    }

    /// <summary>Archiviert die Rule oder führt bei Archived einen No-op aus.</summary>
    public async Task ExecuteAsync(AutomationRuleId ruleId, CancellationToken cancellationToken = default)
    {
        ...
        var result = await store.MutateAsync(validId, rule => rule.Archive(Canonicalize(clock.UtcNow)), cancellationToken).ConfigureAwait(false);

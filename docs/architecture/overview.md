...
FlurNetz wird modular aufgebaut. Die physischen Grenzen der vorgesehenen Fachmodule sind jetzt als getrennte Contracts- und Implementierungs-Assemblies angelegt. Identity ist das erste Modul mit einem bewusst minimalen fachlichen Vertical Slice und einer API; Engagement kann intern eine normalisierte Message-Aktivität aufzeichnen und persistieren; Progression besitzt die minimale Domain-Foundation für Community-Fortschritt. Die übrigen Fachmodule enthalten noch keine Fachlogik, fachlichen Entities, Tabellen oder konkreten Events. Mit `FlurNetz.Api` besitzt FlurNetz einen ausführbaren Host. Fachmodule greifen nicht auf fremde Implementierungen oder Tabellen zu; eine spätere Cross-Module-Kommunikation erfolgt über öffentliche Contracts und Events.
...
`CommunityIdentityId`. Die Foundation startet mit `0` XP und unterstützt positive,
überlaufsichere Akkumulation. Es gibt noch keine Engagement→Progression-Kette; weder
Level-Logik noch Persistence, Messaging, Events, Rewards oder API gehören zu diesem Stand.
...
Die Fachmodule verwenden jeweils das Muster `FlurNetz.Modules.<Module>.Contracts` und `FlurNetz.Modules.<Module>`. Die Implementierung darf nur ihr eigenes Contracts-Projekt und ausdrücklich erlaubte technische Infrastruktur referenzieren; Engagement verwendet zusätzlich ausschließlich `Identity.Contracts`, um die bereits aufgelöste Community-Identität zu übernehmen. Progression verwendet zusätzlich ausschließlich `Identity.Contracts`, um den Progressionszustand derselben internen Identität zuzuordnen. Fremde Modulimplementierungen sind ausgeschlossen. Identity ist das erste Referenzmodul mit einem bewusst begrenzten Vertical Slice; Engagement besitzt den ersten internen Recording-Slice; Progression besitzt derzeit nur die Domain-Foundation und noch keine Progression-Kommunikation. Die vollständige Modul-Liste und Umsetzungsreihenfolge stehen in [modules.md](modules.md).

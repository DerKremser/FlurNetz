...
genau eine interne `CommunityIdentityId`. `InventoryQuantity` ist immutable, auf `long`
basierend und nicht-negativ; Additionen akzeptieren ausschließlich positive Mengen und schützen
vor Overflow, Entnahmen verhindern einen Unterbestand und können den Bestand exakt auf null
reduzieren. `Inventory.Contracts` bleibt leer. Persistence, Messaging, Rewards-/Shop-Anbindung,
Item-Katalog, API und Worker gehören noch nicht zur Foundation. Details stehen in
